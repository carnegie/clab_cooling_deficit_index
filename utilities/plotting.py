...
def plot_exposure_contour(configurations, exposure_function, ac_data, x_y_ranges, country=None, name_tag='exposure_contour', control=False):
    ...
    if not country:
        ...
        contour_plot.add_title('Exposure times CDD for {0}'.format(country))

    contour_plot.add_legend()
    contour_plot.save_figure()

def plot_gdp_increase_map(gdp_cdd_data, future_scenario, exposure_func):
    """
    Plot annual average GDP growth for historical and constant experienced CDD
    """
    gdp_increase_plot = GDPIncreaseMap(None, gdp_cdd_data, 'gdp_increase_map_{0}'.format(future_scenario), future_scenario)
...
def plot_gdp_increase_scatter(configurations, gdp_cdd_data, future_scenario):
    ...
    gdp_increase_scatter.plot_scatter()
    ...
    gdp_increase_scatter.label_countries()
